...
| `data/CSREnrollmentService.kt` | Quick Connect: GET `/Marti/api/tls/config` -> POST CSR to `/Marti/api/tls/signClient/v2` -> build PKCS#12 with a random 32-char passphrase -> save `.p12` + `ca-<host>-<user>.pem`. Port default 8446, Basic auth, `HttpURLConnection`. `Config.trustSelfSigned` gates trust-all (default `false` here, but see [security-audit.md](security-audit.md)). | `EnrollServerScreen`, `ServerQrScanScreen`, `MainActivity` |
