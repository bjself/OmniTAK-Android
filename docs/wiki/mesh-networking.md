...
| `data/MeshtasticProtoParser.kt` | Hand-rolled FromRadio decoder plus the shared varint / fixed / length helpers used by every other protobuf parser. | `skipField` clamps to the buffer end when a declared length exceeds the remaining bytes; never assume a parser loop terminates without that guard. |
...
| `data/TakPacketParser.kt`, `TakPacketSerializer.kt` | Meshtastic ATAK-plugin TAKPacket v1 (portnum 72 / 257). PLI to `a-f-G-U-C`, GeoChat to `b-t-f`. Unishox2 compressed strings handled with `runCatching`. | `uid = contact.device_callsign` from the sender, so any node can claim any UID. `AtakPluginParser` passes `xmlDetail` through `CotDetailSanitizer` before it reaches `rawXml`. |
...
| `data/AdminMessageParser.kt`, `AdminMessageSerializer.kt` | Meshtastic AdminMessage read (`get_owner/get_channel/get_config` responses) and write (`set_owner`, `set_config`, `set_channel`). Admin frames are addressed to the radio's own node number, never broadcast, so PSKs never go over the air. | `buildSetPrimaryChannel(name, current)` echoes the PSK and flags the radio reported and returns null when no PSK is known; `DevicePushPlanner` skips the rename in that case. `set_config` submessages are still single-field and Meshtastic replaces the whole struct on set. |
...
| `domain/MeshCoreManager.kt` | APP_START + DEVICE_QUERY + GET_BATTERY handshake, 2.5 s `GET_NEXT_MSG` poll, frame fan-out, pubkey DMs, `applyChannel`. | `sendCoTOverMesh` updates the advert only for the operator's own PLI (`selfUidProvider`, set by `OmniTAKApp`); relayed contacts are dropped because MeshCore cannot carry third-party positions. |
...
| `domain/MeshServerRelay.kt` | Gateway mode, off by default (`relayGatewayEnabled`). MESH to server with 5 s dedup; server to mesh for `a-*`, `b-t-f`, `b-m-p-*` with 30 s per-uid throttle. Pure `relayTarget` / `admitForward` are unit tested. | Forwards attacker-chosen UIDs (inherent to unauthenticated mesh CoT); `xmlDetail` is sanitized before it reaches `rawXml`. |
...
- `broadcastOverMesh = true`: own position goes out on the mesh whenever a radio is connected, and group-room server chats are re-broadcast on the mesh as all-users GeoChat (DMs are not, see `ChatMeshFanout`).
